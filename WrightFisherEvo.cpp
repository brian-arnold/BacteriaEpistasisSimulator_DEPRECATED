...
void WrightFisherEvo(int &Loci, int &Individuals, int &Generations, double &RecombinationRate, double &GeomTractLength, double &Va, double &Vi, int &GenomeSize, int &SegSize, int &Sim)
{
    ...
    // This vector is used in printing summary statistics everytime diversity decays by 10% from initial value
    ...
    string TempSimFileName2 = "EpistasisTable" + boost::lexical_cast<string>(Sim);
    ofstream out2 ;
    out2.open(TempSimFileName2.c_str()) ;
    ...
    InitializePositions(Positions, Loci, SegSize) ;
    ...
    //out << "EpistasisTable_s\n" ;
    for(i=0; i<(Loci*(Loci-1)/2); i++){
        out2 << EpistasisTable[i] << " " ;
    }
    out2 << "\n" ;
    ...
    out << "Dprime_BY_DIST_gen0" << "\t" <<  "ThetaDecay_1.0" << "\n" ;
    for(i=0; i<(L-1); i++){
    ...
    out << "ThetaPi_gen0" << "\t" <<  "ThetaDecay_1.0" << "\t" << ThetaPiStart << "\n" ;
    ...
    out << "Wmean_gen0" << "\t" <<  "ThetaDecay_1.0" << "\t" << Wmean << "\n" ;
    out << "Wvar_gen0" << "\t" <<  "ThetaDecay_1.0" << "\t" << Wvar << "\n" ;
    out << "Wskew_gen0" << "\t" <<  "ThetaDecay_1.0" << "\t" << Wskew << "\n" ;
    out << "WabsDist_gen0" << "\t" <<  "ThetaDecay_1.0" << "\n" ;
    for(ind=0; ind<Individuals; ind++){
        out << WAbs[ind] << " " ;
    }
    out << "\n" ;
