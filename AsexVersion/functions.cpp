...
void InitializeEpistasisTable(int L, double * ET, int sim)
{
    int i ;
    //double two = 2.0 ;
    ...
    string FileName = "EpistasisTable" + boost::lexical_cast<string>(sim);
...
void InitializePositions(int * Pos, int L, int sim)
{
    int i,j ;
    vector<int> PreviousPos ;
    int temp,flag ;
    
    string FileName = "Results" + boost::lexical_cast<string>(sim);
    ifstream fin(FileName.c_str()) ;
...
double MarginalFitness(Hap * p, int &L, double &VaFit, double * ET)
{
    ...
    for(i=0; i<(L-1); i++){
        for(j=(i+1); j<L; j++){
            //cout << ET[k] << "\n" ;
            //cout << ((*(p+i)).Eff * (*(p+j)).Eff) << "\n" ;
            //cout << ET[k]*((*(p+i)).Eff * (*(p+j)).Eff) << "\n" ;
